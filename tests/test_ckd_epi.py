...
def test_ckd_epi_female_low_creatinine():
    params = CKDEPIParams(age=50, is_female=True, serum_creatinine=0.6, renal_function_stable=True)
...
def test_ckd_epi_male_high_creatinine():
    params = CKDEPIParams(age=65, is_female=False, serum_creatinine=2.0, renal_function_stable=True)
    res = calculate_ckd_epi(params)
...
def test_ckd_epi_extreme_age():
    ...
    params = CKDEPIParams(age=90, is_female=True, serum_creatinine=1.4, renal_function_stable=True)
    res = calculate_ckd_epi(params)
    assert res.value < 45 # Old age significantly reduces eGFR

def test_ckd_epi_kidney_failure():
    params = CKDEPIParams(age=60, is_female=False, serum_creatinine=8.0, renal_function_stable=True)
    res = calculate_ckd_epi(params)
    assert res.value < 15
