...
def test_cockcroft_gault_healthy_young_male():
    ...
    params = CockcroftGaultParams(age=30, weight=80.0, weight_type="actual", is_female=False, serum_creatinine=1.0)
    res = calculate_cockcroft_gault(params)
    assert 120 <= res.value <= 125 
    assert "actual body weight" in res.interpretation

def test_cockcroft_gault_elderly_low_weight_female():
    # 85yo female, 45kg, elevated creatinine (2.0) indicates severe impairment
    params = CockcroftGaultParams(age=85, weight=45.0, weight_type="actual", is_female=True, serum_creatinine=2.0)
    res = calculate_cockcroft_gault(params)
    assert res.value < 15
    assert "acute kidney injury" in res.interpretation

def test_cockcroft_gault_middle_aged_impaired():
    # 55yo male, 100kg, moderately high creatinine (1.8)
    params = CockcroftGaultParams(age=55, weight=100.0, weight_type="actual", is_female=False, serum_creatinine=1.8)
    res = calculate_cockcroft_gault(params)
